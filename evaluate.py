...
def evaluate(
    ...
    """
    Evaluate a trained FauRPPGDeepFakeRecognizer on the val or test split.
    """
    ...
    # same seed=42 as in train.py to get identical splits
        ...
        for batch_idx, (x, y) in enumerate(eval_loader):
            ...
            for m in metrics.values():
                if isinstance(m, MulticlassConfusionMatrix):
                    m.update(logits, y)
                elif isinstance(m, MulticlassAUROC):
                    m.update(probs, y)
                else:
                    m.update(logits, y)
    ...
    typer.echo("\n" + "=" * 50)
    typer.echo(f"  EVALUATION RESULTS ({split} split)")
    typer.echo("=" * 50)
