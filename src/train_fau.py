...
from src.models.fau_lightning import FAUDeepFakeRecognizer
...
def train(
    ...
    batch_size: int = typer.Option(32, "--batch_size", "-bs", help='Размер батча'),
    ...
    """
    Обучение FAUDeepFakeRecognizer на видеодатасетах.
    """
    ...
    if load_from_pretrain is not None:
        if not os.path.exists(load_from_pretrain):
            typer.echo(f"❌ Чекпоинт не найден: {load_from_pretrain}")
            raise FileNotFoundError(load_from_pretrain)
        typer.echo(f"🔄 Возобновление из: {load_from_pretrain}")
    ...
    typer.echo(f"📂 Конфиг: {config_path}")
    ...
    lit_model = FAUDeepFakeRecognizer(model_params=model_cfg, **train_cfg)

    typer.echo("\n🔍 Trainable params:")
    trainable = [name for name, p in lit_model.model.named_parameters() if p.requires_grad]
    typer.echo(f"  {len(trainable)} trainable layers")
    ...
    trainer = pl.Trainer(callbacks=callbacks, strategy="ddp_find_unused_parameters_true", **trainer_cfg)
    typer.echo("🚀 Запуск обучения FAUDeepFakeRecognizer...")
